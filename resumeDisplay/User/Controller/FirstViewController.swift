...
class FirstViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row == 0 {
            myCell.slideDescLabel.text = "Leadership"
            myCell.slideTitleLabel.text = "-"
            myCell.slideImageView.image = #imageLiteral(resourceName: "leader")
            myCell.imageBackground.backgroundColor = .secondarySystemFill
        } else if indexPath.row == 1 {
            myCell.slideDescLabel.text = "Problem Solving"
            myCell.slideTitleLabel.text = "-"
            myCell.slideImageView.image = #imageLiteral(resourceName: "puzzle copy")
            myCell.imageBackground.backgroundColor = .secondarySystemFill
        } else if indexPath.row == 2 {
            myCell.slideDescLabel.text = "Time Management"
            myCell.slideTitleLabel.text = "-"
            myCell.slideImageView.image = #imageLiteral(resourceName: "time")
            myCell.imageBackground.backgroundColor = .secondarySystemFill
        } else if indexPath.row == 3 {
            myCell.slideDescLabel.text = ""
            myCell.slideTitleLabel.text = ""
            myCell.slideImageView.image = #imageLiteral(resourceName: "close_button")
            myCell.imageBackground.backgroundColor = .secondarySystemFill
        }
        return myCell
